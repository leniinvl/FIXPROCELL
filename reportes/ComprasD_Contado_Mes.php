...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(98);
            // Title
            $this->Cell(105,10,'COMPRAS (DETALLADAS) AL CONTADO DEL MES '.strtoupper($meses[date(substr($mes,0,2))-1].' del '.$ano),0,0,'C');
...
try {
    ...
    $pdf->Cell(25,5,'Cantidad',0,0,'L',1);
    $pdf->Cell(35,5,'Cod. Barra',0,0,'L',1);
    ...
    if (is_array($listado) || is_object($listado))
    {
        foreach ($listado as $row => $column) {
          ...
          $pdf->setX(9);
          $pdf->Cell(25,5,$column["cantidad"],0,0,'L',1);
          $pdf->Cell(35,5,$column["codigo_barra"],0,0,'L',1);
        ...
        $pdf->Text(10,$get_Y + 30,'TOTAL EXENTO SIN IVA : '.number_format($total_exento, 2, '.', ','));
