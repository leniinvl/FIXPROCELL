...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(115,10,'PRODUCTOS APARTADOS VIGENTES ENTRE EL '.$fecha1.' Y '.$fecha2,0,0,'C');
...
// Page footer
    ...
    $listado = $objApartado->Listar_Apartados_Detalle('FECHAS',$fecha1,$fecha2,2);
    $totales = $objApartado->Listar_Apartados_Totales('FECHAS',$fecha1,$fecha2,2);
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
      ...
      $pdf->Text(10,$get_Y + 10,'TOTAL DE PRODUCTOS APARTADOS FINALIZADOS : '.number_format($total, 2, '.', ','));
      $pdf->Text(250,$get_Y + 45,'PRECIOS EN : '.$moneda);
    }
    $pdf->Output('I','ApartadosD_Finalizados_del_'.$fecha1.'_al_'.$fecha2.'.pdf');
