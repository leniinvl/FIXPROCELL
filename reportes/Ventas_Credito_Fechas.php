...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(110,10,'VENTAS AL CREDITO ENTRE EL '.$fecha1.' Y '.$fecha2,0,0,'C');
...
// Page footer
    ...
    $objVenta =  new Venta();
    $listado = $objVenta->Listar_Ventas('FECHAS',$fecha1,$fecha2,2,$idsucursal);
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->Text(10,$get_Y + 10,'VALOR TOTAL EN VENTAS AL CREDITO ENTRE FECHAS : '.number_format($total, 2, '.', ','));
