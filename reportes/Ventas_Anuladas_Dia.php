...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(105,10,'VENTAS ANULADAS DEL DIA '.strtoupper($dias[date('w')])." ".strtoupper(date('d'))." DE ".strtoupper(
...
// Page footer
    ...
    $objVenta =  new Venta();
    $listado = $objVenta->Listar_Ventas('HOY','','', 0, $idsucursal);
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        if($tipo_pago == '1')
        {
            ...
            $pdf->Cell(70,5,$column["cliente"],0,0,'L',1);
        ...
        $pdf->Text(10,$get_Y + 10,'MONTO TOTAL EN VENTAS ANULADAS : '.number_format($total, 2, '.', ','));
        //$pdf->Text(10,$get_Y + 15,'PRECIOS EN : '.$moneda);
    }


    $pdf->Output('I','Ventas_Anuladas_del_'.date('d/m/Y').'.pdf');
