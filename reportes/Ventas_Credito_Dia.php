...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(110,10,'VENTAS A CREDITO DEL DIA '.strtoupper($dias[date('w')])." ".strtoupper(date('d'))." DE ".strtoupper(
...
// Page footer
   ...
   session_set_cookie_params(60*60*24*365); session_start();
    ...
    $objVenta =  new Venta();
    $listado = $objVenta->Listar_Ventas('HOY','','',2,$idsucursal);
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->Text(10,$get_Y + 10,'VALOR TOTAL EN VENTAS AL CREDITO : '.number_format($total, 2, '.', ','));
        //$pdf->Text(10,$get_Y + 15,'PRECIOS EN : '.$moneda);
    }


  $pdf->Output('I','Ventas_Credito_del_'.date('d/m/Y').'.pdf');
