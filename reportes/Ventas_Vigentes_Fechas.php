...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(105,10,'VENTAS VIGENTES ENTRE EL '.$fecha1.' Y '.$fecha2,0,0,'C');
...
// Page footer
    ...
    $objVenta =  new Venta();
    $listado = $objVenta->Listar_Ventas('FECHAS',$fecha1,$fecha2,1,$idsucursal);
...
try {
    ...
    $pdf->Cell(65,5,'Tipo Pago',0,0,'L',1);
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->Text(10,$get_Y + 10,'VALOR TOTAL INGRESADO POR VENTAS ENTRE DICHAS FECHAS : '.number_format($total, 2, '.', ','));
