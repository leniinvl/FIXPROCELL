...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(115,10,'DETALLE DE VENTAS VIGENTES ENTRE EL '.$fecha1.' Y '.$fecha2,0,0,'C');
...
// Page footer
    ...
    $objVenta =  new Venta();
    $listado = $objVenta->Listar_Ventas_Detalle('FECHAS',$fecha1,$fecha2,1, $idsucursal);
    $totales = $objVenta->Listar_Ventas_Totales('FECHAS',$fecha1,$fecha2,1, $idsucursal);
    ...
    foreach ($totales as $row => $column) {
      ...
      $total_transferencia = $column['total_transferencia'];
    }
...
try {
    ...
    if (is_array($listado) || is_object($listado))
  {
    ...
    $pdf->Text(10,$get_Y + 25,'VALOR POR TRANSFERENCIA : '.number_format($total_transferencia, 2, '.', ','));
    $pdf->Text(10,$get_Y + 30,'VALOR EXENTO : '.number_format($total_exento, 2, '.', ','));
    $pdf->Text(10,$get_Y + 35,'VALOR EN DESCUENTOS : '.number_format($total_descuento, 2, '.', ','));
    $pdf->Text(10,$get_Y + 40,'***GANANCIA TOTAL POR VENTAS : '.number_format($utilidad, 2, '.', ','));
    //$pdf->Text(250,$get_Y + 45,'PRECIOS EN : '.$moneda);
  }


    $pdf->Output('I','Ventas_Vigentes_del_'.$fecha1.'_al_'.$fecha2.'.pdf');
