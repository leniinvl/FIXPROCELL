...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(105,10,'DETALLE DE VENTAS ANULADAS DEL DIA '.strtoupper($dias[date('w')])." ".strtoupper(date('d'))." DE ".strtoupper(
...
// Page footer
  ...
  //  $mes = isset($_GET['mes']) ? $_GET['mes'] : '';
   ...
   session_set_cookie_params(60*60*24*365); session_start();
    ...
    $objVenta =  new Venta();
    $listado = $objVenta->Listar_Ventas_Detalle('HOY','','',0,$idsucursal);
    $totales = $objVenta->Listar_Ventas_Totales('HOY','','',0,$idsucursal);
...
try {
  ...
  $pdf->Output('I','VentasD_Anuladas_del_'.date('d/m/Y').'.pdf');
