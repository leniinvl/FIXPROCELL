<?php
	...
	$pdf->AddPage();
		...
		$pdf->SetY($get_YH + 5);
		$pdf->SetX(5);
		$pdf->MultiCell(65, 3, 'CLIENTE : '.$nombre_cliente, 0,'L',0 ,1);
		$get_YC = $pdf->GetY();
		$pdf->SetXY(5,$get_YC + 1);
		$pdf->MultiCell(60, 3, 'INGRESO : '.$fecha_ingreso, 0,'L',0 ,1);
		$pdf->Text(5, $get_YC  + 7, '---------------------------------------------------------------------');

		$pdf->SetFont('Arial', 'B', 9.2);
		$pdf->Text(13, $get_YC  + 12, 'INFORMACION DEL APARATO');
		$pdf->SetFont('Arial', '', 9.2);
		$pdf->Text(13, $get_YC  + 18, 'Aparato: '.$aparato);
		$pdf->Text(13, $get_YC  + 23, 'Marca: '.$nombre_marca);
		$pdf->Text(13, $get_YC  + 29, 'Modelo: '.$modelo);
		$pdf->Text(13, $get_YC  + 34, 'Serie: '.$serie_aparato);
		$pdf->SetFont('Arial', 'B', 9.2);
		$pdf->Text(9.5, $get_YC  + 42, 'AVERIA: ');

		$get_XA = $pdf->GetX();
		$get_YA = $pdf->GetY();
		$pdf->SetFont('Arial', '', 9.2);
		$pdf->setXY($get_XA - 2, $get_YA + 40);
		$pdf->MultiCell(60, 4.2, $averia, 0,'L',0 ,1);
		...
		$pdf->SetFont('Arial', 'B', 9.2);
		$pdf->Text($get_XL - 1, $get_YL  + 5, 'Fec. Reparacion: ' . $fecha_alta);
		...
		$pdf->Text($get_XL - 1, $get_YL  + 30, 'COSTO PARCIAL : ' . number_format($parcial_pagar, 2, '.', ','));
		$pdf->SetFont('Arial', 'B', 12);
		$pdf->Text($get_XL - 1, $get_YL  + 36, 'TOTAL : $ ' . number_format($total, 2, '.', ','));
		...
		$pdf->MultiCell(55, 4.2, utf8_decode('Este ticket es su respaldo para cualquier reclamo por los equipos
		reparados. Toda reparacion tiene un mes de garantia'), 0,'C',0 ,1);
