...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(105,10,'APARTADOS FINALIZADOS DEL DIA '.strtoupper($dias[date('w')])." ".strtoupper(date('d'))." DE ".strtoupper(
...
// Page footer
  ...
  //  $mes = isset($_GET['mes']) ? $_GET['mes'] : '';
   // $mes = DateTime::createFromFormat('m/Y', $mes)->format('m-Y');
    ...
    $listado = $objApartado ->Listar_Apartados('HOY','','',2);
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->Text(10,$get_Y + 10,'MONTO TOTAL INGRESADO POR ABONOS DE APARTADO ESTE DIA '.number_format($total_abonado, 2, '.', ','));
        $pdf->Text(10,$get_Y + 15,'PRECIOS EN : '.$moneda);
    }


    $pdf->Output('I','Apartados_Finalizados_del_'.date('d/m/Y').'.pdf');
