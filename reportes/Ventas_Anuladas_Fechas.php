...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(105,10,'VENTAS ANULADAS ENTRE EL '.$fecha1.' Y '.$fecha2,0,0,'C');
...
// Page footer
    ...
    $objVenta =  new Venta();
    $listado = $objVenta->Listar_Ventas('FECHAS',$fecha1,$fecha2,0,$idsucursal);
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->Text(10,$get_Y + 10,'MONTO TOTAL EN VENTAS ANULADAS ENTRE DICHAS FECHAS : '.number_format($total, 2, '.', ','));
        //$pdf->Text(10,$get_Y + 15,'PRECIOS EN : '.$moneda);
    }


     $pdf->Output('I','Ventas_Anuladas_del_'.$fecha1.'_al_'.$fecha2.'.pdf');
