...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(105,10,'APARTADOS VIGENTES DEL DIA '.strtoupper($dias[date('w')])." ".strtoupper(date('d'))." DE ".strtoupper(
...
// Page footer
  ...
  //  $mes = isset($_GET['mes']) ? $_GET['mes'] : '';
   // $mes = DateTime::createFromFormat('m/Y', $mes)->format('m-Y');
    ...
    $listado = $objApartado ->Listar_Apartados('HOY','','',1);
...
try {
    ...
    $pdf->Output('I','Apartados_Vigentes_del_'.date('d/m/Y').'.pdf');
