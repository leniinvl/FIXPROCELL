...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(110,10,'DETALLE DE VENTAS AL CONTADO DEL DIA '.strtoupper($dias[date('w')])." ".strtoupper(date('d'))." DE ".strtoupper(
...
// Page footer
   ...
   // $mes = DateTime::createFromFormat('m/Y', $mes)->format('m-Y');
    ...
    $objVenta =  new Venta();
    $listado = $objVenta->Listar_Ventas_Detalle('HOY','','',1,$idsucursal);
    $totales = $objVenta->Listar_Ventas_Totales('HOY','','',1,$idsucursal);
...
try {
    ...
    $pdf->Output('I','VentasD_Contado_del_'.date('d/m/Y').'.pdf');
