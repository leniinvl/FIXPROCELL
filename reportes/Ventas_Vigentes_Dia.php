...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(105,10,'VENTAS VIGENTES DEL DIA '.strtoupper($dias[date('w')])." ".strtoupper(date('d'))." DE ".strtoupper(
...
// Page footer
  ...
  //  $mes = isset($_GET['mes']) ? $_GET['mes'] : '';
   // $mes = DateTime::createFromFormat('m/Y', $mes)->format('m-Y');
    ...
    $objVenta =  new Venta();
    $listado = $objVenta->Listar_Ventas('HOY','','',1,$idsucursal);
...
try {
    ...
    $pdf->Cell(34,5,'No. Venta',0,0,'L',1);
    $pdf->Cell(35,5,'Comprobante',0,0,'L',1);
    $pdf->Cell(40,5,'No. Comprobante',0,0,'L',1);
    $pdf->Cell(45,5,'Fecha y Hora',0,0,'L',1);
    $pdf->Cell(75,5,'Cliente',0,0,'L',1);
    $pdf->Cell(60,5,'Tipo Pago',0,0,'L',1);
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        if($tipo_pago == '1')
        {
            ...
            $pdf->Cell(45,5,$c_fecha_venta,0,0,'L',1);
            $pdf->Cell(75,5,$column["cliente"],0,0,'L',1);
            $pdf->Cell(60,5,$tipo_pago,0,0,'L',1);
            $pdf->Cell(20,5,$column["total"],0,0,'C',1);
        ...
        $pdf->Text(10,$get_Y + 10,'MONTO TOTAL INGRESADO POR VENTAS: '.number_format($total, 2, '.', ','));
        //$pdf->Text(10,$get_Y + 15,'PRECIOS EN : '.$moneda);
    }


    $pdf->Output('I','Ventas_Vigentes_del_'.date('d/m/Y').'.pdf');
