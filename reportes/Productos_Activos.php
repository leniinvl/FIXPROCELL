...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(105);
            // Title
            $this->Cell(105,10,'Reporte de productos activos en almacen',0,0,'C');
...
// Page footer
    ...
    $listado = $objProducto->Listar_Productos_Activos($idsucursal, $idcategoria);
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->Text(10,$get_Y + 10,'TOTAL DE PRODUCTOS ACTIVOS: '.number_format($total, 2, '.', ','));
    }


    $pdf->Output('I','Productos_Activos.pdf');
