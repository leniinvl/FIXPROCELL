<?php
	...
	if (!empty($_POST))
	{
      ...
      $fecha_retiro = DateTime::createFromFormat('d/m/Y H:i:s', $fecha_retiro)->format('Y-m-d H:i:s');
		...
		$funcion->Insertar_Apartado($fecha_retiro,$sumas,$iva,$exento,$retenido,$descuento,$total,$abonado,
		$restante,$son_letras, $idcliente,$_SESSION['user_id']);
		  	...
		  	if($fecha_vence=='')
				{
		  	 	...
		  	 	$funcion->Insertar_DetalleApartado($idproducto,$cantidad,$precio_unitario,$exentos,$descuento,
		  	 	$fecha_vence,$importe);
