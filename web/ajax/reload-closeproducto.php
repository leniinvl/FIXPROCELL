<div class="panel-body">
	<div class="row">
		<div class="col-md-12">

			<!-- Widget with rounded icon -->
			<div class="panel">
				<div class="panel-body text-center">
					<div class="icon-object border-warning-400 text-primary-400"><i class="icon-warning22 icon-3x text-warning-400"></i>
					</div>
					<h2 class="no-margin text-semibold">INVENTARIO NO VIGENTE </h2>
					<span class="text-uppercase text-size-mini text-muted">Debe abrir un nuevo inventario para
					el mes correspondiente y poder administrar sus productos </span> <br><br>
				
					<button id="btnInventario" onclick="IrInventario()" type="button" class="btn btn-success heading-btn"> 
					<i class="icon-forward"></i>  Dirigir a Inventario</button>
					
				</div>
			</div>
			<!-- /widget with rounded icon -->
		</div>
	</div>

	<script type="text/javascript" src="web/custom-js/producto.js"></script>
