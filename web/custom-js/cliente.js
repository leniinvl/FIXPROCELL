...
function openCliente(action, idcliente, codigo_cliente, nombre_cliente, numero_nit, numero_nrc, direccion, numero_telefono, email, giro ,limite_credito ,estado)
 {
    ...
    $('#modal_iconified').on('shown.bs.modal', function () {
     ...
     if (action == 'nuevo'){
      ...
      $('#txtEmail').val(email);
      $('#txtGiro').val(giro).trigger("change");
...
function enviar_frm()
{
  ...
  if(numero_nit.length != 10 && numero_nit.length != 13){
    ...
    swal("Alerta de datos!","Valide Cédula o Ruc del cliente sean correctos","warning");
