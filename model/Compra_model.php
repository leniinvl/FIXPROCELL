<?php
	...
	class CompraModel extends Conexion
	{
		...
		public static function Listar_Compras($criterio,$date,$date2,$estado,$pago,$idsucursal)
		{
			$dbconec = Conexion::Conectar();

			try
			{
				$query = "CALL sp_consulta_compras(:criterio,:date,:date2,:estado,:pago,:idsucursal);";
				$stmt = $dbconec->prepare($query);
		...
		public static function Count_Compras($criterio,$date,$date2,$idsucursal)
		{
			$dbconec = Conexion::Conectar();

			try
			{
				$query = "CALL sp_count_compras(:criterio,:date,:date2,:idsucursal);";
				$stmt = $dbconec->prepare($query);
				$stmt->bindParam(":criterio",$criterio);
				$stmt->bindParam(":date",$date);
				$stmt->bindParam(":date2",$date2);
		...
		$sumas, $iva, $exento, $retenido, $total, $sonletras, $idsucursal)
		{
			$dbconec = Conexion::Conectar();
			try
			{
				$query = "CALL sp_insert_compra(:fecha_compra, :idproveedor, :tipo_pago, :numero_comprobante, :tipo_comprobante,
				:fecha_comprobante, :sumas, :iva, :exento, :retenido, :total, :sonletras, :idsucursal)";
				$stmt = $dbconec->prepare($query);
