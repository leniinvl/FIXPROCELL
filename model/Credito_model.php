<?php
	...
	class CreditoModel extends Conexion
	{
		...
		public static function Listar_Creditos($criterio, $idsucursal)
		{
			$dbconec = Conexion::Conectar();

			try
			{
				$query = "CALL sp_view_creditos(:criterio,:idsucursal);";
				$stmt = $dbconec->prepare($query);
		...
		public static function Listar_Creditos_Espc($criterio)
		{
			$dbconec = Conexion::Conectar();
    ...
    public static function Listar_Abonos_Credito($criterio)
    {
      $dbconec = Conexion::Conectar();
      ...
      try
      {
		...
		public static function Reporte_Abonos($fecha,$fecha2,$idsucursal)
		{
			...
			try
			{
				$query = "CALL sp_view_total_abonos_fechas(:fecha,:fecha2,:idsucursal);";
				$stmt = $dbconec->prepare($query);
    ...
    public static function Listar_Abonos_All($idsucursal)
    {
      ...
      try
      {
        $query = "CALL sp_view_all_abonos(:idsucursal);";
        $stmt = $dbconec->prepare($query);
		$stmt->bindParam(":idsucursal",$idsucursal);
        $stmt->execute();
        $count = $stmt->rowCount();
      ...
      try {
        ...
        echo "Error al cargar el listado";
    		...
    		public static function Count_Creditos($idsucursal)
    		{
    			...
    			try
    			{
    				$query = "CALL sp_count_creditos(:idsucursal);";
    				$stmt = $dbconec->prepare($query);
					$stmt->bindParam(":idsucursal",$idsucursal);
    				$stmt->execute();
    				$count = $stmt->rowCount();
